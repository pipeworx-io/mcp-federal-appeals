...
Federal appellate dockets — the thirteen circuits, the Federal Circuit and the Supreme Court. 2.2 million dockets. This is the *case* on appeal; the written opinion that results is in the `court-listener` pack.
...
| Tool | Returns |
|---|---|
| `federal_appeal_search` | Dockets matching a party name |
| `federal_appeal_recent` | Recent dockets, newest first |
...
`tools/list` at `https://gateway.pipeworx.io/federal-appeals/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/federal_appeal_search \
  -H 'Content-Type: application/json' \
  -d '{"party":"Oracle","limit":5}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/federal_appeal_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "federal-appeals": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-federal-appeals"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-federal-appeals
```
...
```
ask_pipeworx({ question: "your question about Federal Appeals data" })
```
